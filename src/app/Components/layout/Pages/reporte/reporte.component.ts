...
export class ReporteComponent implements OnInit,AfterViewInit {
  ...
  columnasTabla:string[]=['fechaRegistro','numeroDocumento','tipoPago','total','servicio','cantidad','precio','totalVenta'];
    ...
    this._ventaServicio.reporte(
      ...
      next:(data)=>{
        if(data.status){
          ...
          ///Recibe la informacion data y en la linea de abajo actualiza la informacion
          this.dataVentaReporte.data=data.value;
        }else{
          this.listaVentasReporte=[];
          ///Recibe la informacion data y como esta mala, lo limpia
  ...
  exportarExcel(){
    ...
    //la linea de arriba es el libro y la de abajo la hoja 
