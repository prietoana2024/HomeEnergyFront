...
@Component({
  selector: 'app-dash-board-v',
  templateUrl: './dash-board-v.component.html',
  styleUrls: ['./dash-board-v.component.css']
})

export class DashBoardVComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._dashboardServicio.resumen().subscribe({
      ...
      next:(data)=>{
        if(data.status){
          //const yo = data.value.filter(persona=> persona.edad >= 18);
